...
from src.meter_ocr.yolo.utils.datasets1 import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams
from src.meter_ocr.yolo.utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr, cv2,
                           increment_path, non_max_suppression, print_args, scale_coords, strip_optimizer, xyxy2xywh)
from src.meter_ocr.yolo.utils.plots import Annotator, colors, save_one_box
from src.meter_ocr.yolo.utils.torch_utils import select_device, time_sync
...
def run(
    ...
    # print("device and source", device, source)
...
def region_detection(source_base64):
    ...
    try:
        model_name = "best_business_card.pt"
        class_info = ['logo']
        imgdata = base64.b64decode(source_base64)
        source_img = Path(os.getcwd() + '/vivyacards_app/data/logo.jpg')
        with open(source_img, 'wb') as f:
            f.write(imgdata)
        weights = Path(os.getcwd() + '/vivyacards_app/yolo/' + model_name)
